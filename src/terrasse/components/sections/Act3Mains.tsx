...
export const Act3Mains = () => {
  ...
  return (
    <section ref={ref} id="acte-3" aria-label="Acte III — Les mains" style={{ position: 'relative', background: 'var(--ltdr-black)', overflow: 'hidden', padding: 'clamp(16vh, 20vh, 24vh) 0 clamp(18vh, 22vh, 26vh)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 var(--pad-x)' }}>
        ...
        <div style={{ maxWidth: 620, marginLeft: 'auto' }}>
          ...
          <SectionText>
            Nous les avons rouvertes. À la main. Pierre par pierre, cep par cep. Pas de machine sur ces pentes — seulement le geste, répété,
            exigeant.
          </SectionText>
        </div>

        {/* image (révélation gris→couleur au scroll) — décalée à gauche */}
        <figure ref={figRef} style={{ width: 'clamp(260px, 46vw, 600px)', margin: 'clamp(14vh, 18vh, 22vh) auto 0 0' }}>
          <Pic
            photo={PHOTOS.mains}
            sizes="(max-width: 900px) 75vw, 46vw"
            style={{ filter: 'grayscale(var(--desat, 1)) brightness(calc(0.7 + 0.3 * (1 - var(--desat, 1))))' }}
          />
          <figcaption style={{ marginTop: 12, fontSize: 10, letterSpacing: '0.26em', textTransform: 'uppercase', color: 'var(--ltdr-ivory-35)' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 'clamp(40px, 8vw, 120px)', padding: 'clamp(14vh, 18vh, 22vh) 24px 0' }}>
