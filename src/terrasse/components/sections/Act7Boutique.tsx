...
export const Act7Boutique = () => {
  ...
  return (
    <section ref={ref} id="acte-7" aria-label="Acte VII — La rencontre" style={{ position: 'relative' }}>
      {/* pont tonal : fond sombre de la bouteille → ivoire, sans coupe sèche */}
      <div
        aria-hidden="true"
        style={{ height: '22vh', background: 'linear-gradient(to bottom, var(--ltdr-black-bottle), #1c1712 35%, var(--ltdr-ivory))' }}
      />
      <div id="ltdr-light" style={{ background: 'var(--ltdr-ivory)', color: 'var(--ltdr-black)', padding: '4vh var(--pad-x) 14vh' }}>
        <div style={{ maxWidth: 1080, margin: '0 auto' }}>
          <p className="acte-kicker" style={{ color: 'var(--ltdr-gold-dark)' }}>
            Acte VII — La rencontre
          </p>
          <h2 className="display" style={{ margin: '0 0 14px', fontSize: 'var(--fs-h2)', lineHeight: 1.05, letterSpacing: '-0.01em' }}>
            Deux cuvées.
            <br />
            Pas une de plus.
          </h2>
          <p style={{ margin: '0 0 9vh', fontSize: 'clamp(13px, 1vw, 15px)', letterSpacing: '0.04em', color: 'var(--ltdr-ink-strong)' }}>
            Livraison offerte dès 300&nbsp;€ en France métropolitaine.
          </p>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: 'clamp(28px, 4vw, 56px)' }}>
            <article style={{ display: 'grid', gap: 0, alignContent: 'start' }}>
              <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--ltdr-black)' }}>
                <Pic
                  photo={PHOTOS.cuvee2023}
                  sizes="(max-width: 900px) 90vw, 45vw"
                  style={{ height: 'clamp(300px, 36vw, 460px)', objectFit: 'cover', opacity: 0.95, filter: 'var(--grade-photo)' }}
                />
                <span
                  style={{
                    position: 'absolute',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 16, marginTop: 24 }}>
                <h3 className="display" style={{ margin: 0, fontWeight: 500, fontSize: 'clamp(24px, 2.4vw, 34px)' }}>
                  Cuvée 2023
                </h3>
                <span className="display" style={{ fontSize: 22, color: 'rgba(10,9,8,0.4)', textDecoration: 'line-through' }}>
                  25 €
                </span>
              </div>
              <p style={{ margin: '10px 0 0', fontSize: 'clamp(13px, 1vw, 15px)', lineHeight: 1.75, color: 'var(--ltdr-ink-strong)' }}>
                Première cuvée née des terrasses rouvertes. Partie en quelques semaines — la rareté n'est pas un argument, c'est un fait.
              </p>
            </article>
            ...
            <article style={{ display: 'grid', gap: 0, alignContent: 'start' }}>
              <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--ltdr-black)' }}>
                <Pic
                  ...
                  sizes="(max-width: 900px) 90vw, 45vw"
                  style={{ height: 'clamp(300px, 36vw, 460px)', objectFit: 'cover', objectPosition: 'center 30%', filter: 'var(--grade-photo)' }}
                />
                <span
                  style={{
                    position: 'absolute',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 16, marginTop: 24 }}>
                ...
                <span className="display" style={{ fontSize: 26 }}>25 €</span>
              </div>
              <p style={{ margin: '10px 0 0', fontSize: 'clamp(13px, 1vw, 15px)', lineHeight: 1.75, color: 'var(--ltdr-ink-strong)' }}>
                Syrah 100 %, certifiée Ecocert<span style={{ fontSize: 9, verticalAlign: 'super' }}>®</span>. Livraison à la mise en
                bouteille, novembre 2025.
              </p>
              <a
                ref={ctaRef}
                href={SHOP.cuvee2024}
          ...
          <div style={{ marginTop: '16vh', maxWidth: 820 }}>
            <h3 className="display" style={{ margin: '0 0 5vh', fontSize: 'clamp(28px, 3.4vw, 46px)' }}>
